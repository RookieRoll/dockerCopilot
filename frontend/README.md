...
开发服务器访问 **http://localhost:12713** - 开始您的Docker之旅 🚀

生产环境由 Go 服务在 **/manager** 路径提供；构建前端后，请将 `dist/` 内容放入主项目的 `front/` 目录，或使用主项目 CI 自动完成该步骤。
...
`frontend/` 是主项目中的 React 前端源码目录，生产环境由 Go 服务统一提供。前端可以独立启动用于开发，但发布时应通过主项目的构建流程生成 `frontend/dist/`，再同步到根目录 `front/` 供 Go 的 `//go:embed front/*` 使用。
...
构建完成后，使用主项目根目录的 `front/` 作为 Go 服务的生成输入，并执行：

```bash
# 在主项目根目录执行；CI 会自动完成同样的同步
cp -r frontend/dist/. front/
go test ./...
```

Go 服务通过 `/manager` 提供管理页面，前端生产资源使用 `/manager/assets/` 路径。请优先使用根目录 `README.md` 中的完整 Docker 发布说明；`frontend/Dockerfile` 和 `frontend/docker-compose.yml` 仅用于需要单独调试前端时的场景。
